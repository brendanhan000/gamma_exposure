...
def delta_report(chain_dir, ticker, top, expiry_filter):
    ...
    for key, r in cur.items():
        ...
        rows.append((abs(d), d, exp, cp, strike, oi_now, lean))
    ...
    print("=" * 78)
    print("  {:<12}{:<6}{:>9}{:>12}{:>11}   {}".format(
        "expiry", "type", "strike", "dOI", "OI now", "aggressor lean (crude)"))
    rows.sort(reverse=True)
    for _, d, exp, cp, strike, oi_now, lean in rows[:top]:
        print("  {:<12}{:<6}{:>9,.0f}{:>+12,.0f}{:>11,.0f}   {}".format(
            exp, cp, strike, d, oi_now, lean))

    built = sum(d for _, d, *_ in rows if d > 0)
    cut = sum(d for _, d, *_ in rows if d < 0)
    print("-" * 78)
    ...
    print("  stale inventory that may already be hedged. The aggressor lean is a")
    print("  one-snapshot-per-day proxy -- a hint, not open-close data.")
    return 0
